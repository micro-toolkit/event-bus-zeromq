...
function connect(snapshot, subscriber, config, handlers) {
  ...
  log.info(syncStartFrames, 'Started subscriber syncronization for topics %s with sequence %s', topicsFrame, sequence)
...
function snapshotHandler(handlers, subscriber, state) {
  ...
  log.debug(frames, 'Received snapshot command: %s', cmd)
